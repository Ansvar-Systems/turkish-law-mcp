#!/usr/bin/env tsx
/**
 * Database builder for Turkish Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Turkish Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Turkish statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'TR');

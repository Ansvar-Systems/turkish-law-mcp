#!/usr/bin/env tsx
/**
 * Turkish Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'https://www.mevzuat.gov.tr';
const PORTAL_NAME = 'Mevzuat Bilgi Sistemi';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/turkish-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Turkish Law MCP — Data Freshness Check');

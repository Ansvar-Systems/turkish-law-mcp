/**
 * search_eu_implementations — Search EU directives/regulations referenced by Turkish legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_turkish_implementation?: boolean;
  limit?: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as turkish_statute_count
  ...
  if (input.has_turkish_implementation) {
    sql += ' HAVING turkish_statute_count > 0';
  }

/**
 * get_turkish_implementations — Find Turkish statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetTurkishImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface TurkishImplementationResult {
  document_id: string;
...
export async function getTurkishImplementations(
  db: InstanceType<typeof Database>,
  input: GetTurkishImplementationsInput,
): Promise<ToolResponse<TurkishImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as TurkishImplementationResult[];
